...
inline void DatumVariant::to_proto(VariantPB* variantPB) const {
    *variantPB->mutable_type() = TypeDescriptor::from_storage_type_info(_type.get()).to_protobuf();
    if (_value.get().is_null()) {
        return;
    }
    *variantPB->mutable_value() = datum_to_string(_type.get(), _value.get());
}
...
inline Status DatumVariant::from_proto(const VariantPB& variantPB) {
    ...
    _type = get_type_info(TypeDescriptor::from_protobuf(variantPB.type()));
    Datum datum;
    if (!variantPB.has_value()) {
        datum.set_null();
    } else {
        RETURN_IF_ERROR(datum_from_string(_type.get(), &datum, variantPB.value(), nullptr));
    }
    _value.set(datum);
    return Status::OK();
}
