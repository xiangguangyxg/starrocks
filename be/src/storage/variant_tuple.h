...
public:
    ...
    Status from_proto(const TuplePB& tuplePB);
...
inline void VariantTuple::to_proto(TuplePB* tuplePB) const {
    for (const auto& value : _values) {
        value.to_proto(tuplePB->add_values());
    }
}

inline Status VariantTuple::from_proto(const TuplePB& tuplePB) {
    auto size = tuplePB.values_size();
    _values.resize(size);
    for (auto i = 0; i < size; ++i) {
        RETURN_IF_ERROR(_values[i].from_proto(tuplePB.values(i)));
    }
    return Status::OK();
}
