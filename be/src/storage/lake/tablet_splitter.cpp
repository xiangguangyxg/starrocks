...
StatusOr<std::unordered_map<int64_t, MutableTabletMetadataPtr>> TabletSplitter::split(
        TabletManager* tablet_manager, const TabletMetadataPtr& old_tablet_metadata,
        const SplittingTabletInfoPB& splitting_tablet) const {
    ...
    const auto& old_tablet_range = old_tablet_metadata->range();
    for (int32_t i = 0; i < splitting_tablet.new_tablet_ids_size(); ++i) {
        ...
        ASSIGN_OR_RETURN(auto effective_range,
                         tablet_reshard_helper::intersect_range(old_tablet_range, split_ranges[i].range));
        for (auto& rowset_metadata : *new_tablet_new_metadata->mutable_rowsets()) {
            RETURN_IF_ERROR(tablet_reshard_helper::update_rowset_range(&rowset_metadata, effective_range));
