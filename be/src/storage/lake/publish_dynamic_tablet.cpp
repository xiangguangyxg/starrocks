...
TxnLogPtr convert_txn_log_for_dynamic_tablet(const TxnLogPtr& original_txn_log,
    ...
    if (original_txn_log->tablet_id() == base_tablet_metadata->id() && !tablet_splitting) {
...
StatusOr<std::vector<TabletMetadataPtr>> publish_splitting_tablet(TabletManager* tablet_manager,
    ...
    std::vector<TabletMetadataPtr> new_metadatas;
    new_metadatas.reserve(1 + splitting_tablet.new_tablet_ids_size());
    ...
    do {
        ...
        new_metadatas.push_back(std::move(cached_old_tablet_new_metadata));
        ...
        for (auto new_tablet_id : splitting_tablet.new_tablet_ids()) {
            ...
            new_metadatas.push_back(std::move(cached_new_tablet_new_metadata));
        }
    ...
    if (old_tablet_old_metadata_or.status().is_not_found()) {
        ...
        new_metadatas.push_back(std::move(old_tablet_new_metadata_or.value()));
        ...
        for (auto new_tablet_id : splitting_tablet.new_tablet_ids()) {
            ...
            new_metadatas.push_back(std::move(new_tablet_new_metadata_or.value()));
        }
    ...
    // Old tablet
    {
        ...
        new_metadatas.push_back(std::move(old_tablet_new_metadata));
    }
    ...
    for (int64_t new_tablet_index = 0; auto new_tablet_id : splitting_tablet.new_tablet_ids()) {
        ...
        new_metadatas.push_back(std::move(new_tablet_new_metadata));

        ++new_tablet_index;
    }

    for (const auto& new_metadata : new_metadatas) {
        if (!skip_write_tablet_metadata) {
            ...
            tablet_manager->metacache()->cache_aggregation_partition(
                    tablet_manager->tablet_metadata_root_location(new_metadata->id()), true);
        }
    }
