...
Status merge_delvecs(TabletManager* tablet_manager, const std::vector<TabletMergeInfo>& merge_infos,
                     int64_t delvec_version, TabletMetadataPB* new_metadata) {
    ...
    const int64_t merge_file_id = MonotonicNanos();
    FileMetaPB new_delvec_file;
    std::vector<uint64_t> offsets;
    RETURN_IF_ERROR(merge_delvec_files(tablet_manager, old_delvec_files, new_metadata->id(), merge_file_id,
    ...
    for (const auto& info : merge_infos) {
        ...
        for (const auto& [segment_id, page] : metadata->delvec_meta().delvecs()) {
            ...
            new_page.set_version(delvec_version);
            new_page.set_crc32c_gen_version(delvec_version);
    ...
    (*delvec_meta->mutable_version_to_file())[delvec_version] = std::move(new_delvec_file);
    return Status::OK();
}

} // namespace

StatusOr<MutableTabletMetadataPtr> TabletMerger::merge(TabletManager* tablet_manager,
                                                       const std::vector<TabletMetadataPtr>& old_tablet_metadatas,
                                                       const MergingTabletInfoPB& merging_tablet) const {
    ...
    if (is_primary_key(*new_tablet_metadata)) {
        RETURN_IF_ERROR(
                merge_delvecs(tablet_manager, merge_infos, new_tablet_metadata->version(), new_tablet_metadata.get()));
    }
