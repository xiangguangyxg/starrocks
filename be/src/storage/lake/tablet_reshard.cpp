...
CONTINUE_HANDLE_SPLITTING_TABLET:
    ...
    tablet_reshard_helper::apply_publish_fields(old_tablet_new_metadata.get(), new_version, txn_info);
    ...
    TabletSplitter splitter;
    ASSIGN_OR_RETURN(auto split_new_metadatas,
                     splitter.split(tablet_manager, old_tablet_old_metadata, splitting_tablet));
    for (auto& [tablet_id, tablet_metadata] : split_new_metadatas) {
        tablet_reshard_helper::apply_publish_fields(tablet_metadata.get(), new_version, txn_info);
...
CONTINUE_HANDLE_MERGING_TABLET:
    ...
    TabletMerger merger;
    ASSIGN_OR_RETURN(auto new_tablet_metadata, merger.merge(tablet_manager, old_tablet_metadatas, merging_tablet));
    tablet_reshard_helper::apply_publish_fields(new_tablet_metadata.get(), new_version, txn_info);
    tablet_reshard_helper::normalize_merged_delvec_version(new_tablet_metadata.get(), new_version);
...
CONTINUE_HANDLE_IDENTICAL_TABLET:
    ...
    tablet_reshard_helper::apply_publish_fields(new_tablet_new_metadata.get(), new_version, txn_info);
