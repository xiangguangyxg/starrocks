...
void apply_publish_fields(TabletMetadataPB* tablet_metadata, int64_t new_version, const TxnInfoPB& txn_info) {
    DCHECK(tablet_metadata != nullptr);
    tablet_metadata->set_version(new_version);
    tablet_metadata->set_commit_time(txn_info.commit_time());
    tablet_metadata->set_gtid(txn_info.gtid());
}

void normalize_merged_delvec_version(TabletMetadataPB* tablet_metadata, int64_t new_version) {
    DCHECK(tablet_metadata != nullptr);
    if (!tablet_metadata->has_delvec_meta()) {
        return;
    }

    auto* delvec_meta = tablet_metadata->mutable_delvec_meta();
    if (!delvec_meta->version_to_file().empty()) {
        FileMetaPB merged_file = delvec_meta->version_to_file().begin()->second;
        delvec_meta->mutable_version_to_file()->clear();
        (*delvec_meta->mutable_version_to_file())[new_version] = std::move(merged_file);
    }

    for (auto& [segment_id, page] : *delvec_meta->mutable_delvecs()) {
        (void)segment_id;
