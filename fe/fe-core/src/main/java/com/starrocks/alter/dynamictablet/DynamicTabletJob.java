...
public abstract class DynamicTabletJob implements Writable {
    ...
    // Partition id -> <Materialized index id -> Dynamic tablets>
    @SerializedName(value = "dynamicTablets")
    protected final Map<Long, Map<Long, DynamicTablets>> dynamicTablets;

    @SerializedName(value = "parallelTablets")
    protected final long parallelTablets;

    public DynamicTabletJob(long jobId, JobType jobType, long dbId, long tableId,
            Map<Long, Map<Long, DynamicTablets>> dynamicTablets) {
        this.jobId = jobId;
        this.jobType = jobType;
        this.dbId = dbId;
        this.tableId = tableId;
        this.dynamicTablets = dynamicTablets;
        this.parallelTablets = calcParallelTablets();
    }
    ...
    public Map<Long, Map<Long, DynamicTablets>> getDynamicTablets() {
    ...
    private long calcParallelTablets() {
        long parallelTablets = 0;
        for (Map<Long, DynamicTablets> indexToDynamicTablets : dynamicTablets.values()) {
            for (DynamicTablets aDynamicTablets : indexToDynamicTablets.values()) {
                parallelTablets += aDynamicTablets.getParallelTablets();
            }
        }
        return parallelTablets;
    }
